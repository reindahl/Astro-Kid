   ...
   (:action climbUp
	...
	(:action updateDestroyEnd
       ...
       :precondition 	(and  
							(update updateStage1)
							(not 
								(exists (?t - thing ?at ?under - location)
									(or 
										(and 
											(boarder ?at)
											(at ?t ?at) 	
	...
	(:action updateGate
		:parameters  (?gate ?b - location ?col - colour)
		:precondition 	(and
							(update updateStage5)
							(gate ?col ?gate)
							(button ?col ?b)
	...
	(:action updateGateEnd
		:parameters  ()
		:precondition 	(and
							(update updateStage5)
							(not
								(exists (?gate ?b - location ?col - colour)
										(and
											(gate ?col ?gate)
											(button ?col ?b)
											(or
												(and (clear ?b)(closed ?gate))
												(and (not (clear ?b))(not (closed ?gate)))
											)
										)
